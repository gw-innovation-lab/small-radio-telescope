...
#This doesn't work and may never
@app.callback(
...
def motorLocation(home_clicks, alt_for_clicks, alt_rev_clicks, az_for_clicks, az_rev_clicks):
    global homeprev 
    global altforprev
    global altrevprev
    global azforprev
    global azrevprev
    if home_clicks > homeprev or alt_for_clicks > altforprev or alt_rev_clicks > altrevprev or az_for_clicks > azforprev or az_rev_clicks > azrevprev:
        data1 = ser1.readline().decode('ascii')
        identity = data1[0]
        location = data1[3]
        if identity == 0:
            if location < 1003:
                azimuthDec = 90 / location
                az_deg, az_min, az_sec = degreeSpliterRounder(azimuthDec)
                motAz = "{}°{}'".format(az_deg, az_mins)
                return motAz
            if location > 1003:
                azimuthDec = 90 - 90 / (location - 1003)
                az_deg, az_min, az_sec = degreeSpliterRounder(azimuthDec)
                motAz = "{}°{}'".format(az_deg, az_mins)
                return motAz
